...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		// length of Array
		{`len(["asd", 1, 2, 4])`, 4},
		{`let myArr = [1, 2, [1, 2, 3]]; len(myArr[2])`, 3},

		// first element of Array
		{`first([1, 2, 3, 4])`, 1},
		{`let myArr = [1, 2, 3, 4, [0, 1, 2]]; first(myArr[4])`, 0},
		{`first(1)`, "argument to first must be ARRAY, got INTEGER"},

		// last element of Array
		{`last([0, 1, 2, 3])`, 3},
		{`let myArr = [1, 2, 3, 4, [0, 1, 2]]; last(myArr[4])`, 2},
		{`last(1)`, "argument to last must be ARRAY, got INTEGER"},

		// rest elements of Array
		{`rest(rest([0, 1, 2, 3]));`, []int{2, 3}},
		{`rest(1)`, "argument to rest must be ARRAY, got INTEGER"},

		// push element in Array
		{`push(push([0, 1, 2, 3], 4), 5);`, []int{0, 1, 2, 3, 4, 5}},
...
func TestArrayLiterals(t *testing.T) {
	...
	if !ok {
		t.Fatalf("object is not Array. got=%T (%+v)", evaluated,
			evaluated)
	}
